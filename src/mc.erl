...
json_val( Key, Tuple ) when is_tuple(Tuple), tuple_size(Tuple) == 1 -> json_val( Key, element( 1, Tuple ));
json_val( Key, Tuple ) when is_tuple(Tuple) -> json_val( Key, element( 1, Tuple ));
...
localtime( _A ) -> lists:flatten(io_lib:format( "~4..0B-~2..0B-~2..0B ~2..0B:~2..0B:~2..0B", mc:localtime())).
